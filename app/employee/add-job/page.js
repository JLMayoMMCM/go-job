...
export default function AddJobPage() {
  ...
  const checkAuthAndLoadData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUser(userData);
        await Promise.all([loadJobTypes(token), loadJobCategories(token)]);
      } else {
        router.push('/Login');
      }
    } catch (error) {
  ...
  const loadJobTypes = async (token) => {
    try {
      ...
      if (response.ok) {
        ...
        setJobTypes(data);
        // Set first job type as default if available
        if (data.length > 0) {
          setFormData(prev => ({ ...prev, jobTypeId: data[0].job_type_id.toString() }));
        }
      } else {
      ...
      setError('Error loading job types');
    }
  };
  ...
  const loadJobCategories = async (token) => {
    try {
      ...
      setError('Error loading job categories');
    }
  };
